...
def test_corpus_label_regression(init_database, client, upload_audio,
    ...
    from persephone_api.db_models import Label

    # Create a label first before the corpus uses it
    label_A = "A"
    current_label = Label(label=label_A)
    ...
    # Create mock transcription uploads
    response = upload_transcription(label_A, filename="a.phonemes")
    assert response.status_code == 201
    transcription_response_data = json.loads(response.data.decode('utf8'))
    transcription_id_a = transcription_response_data['id']
